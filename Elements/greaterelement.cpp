...
class GreaterWidget: public QGraphicsWidget
{
    qreal width, height;
    LDElement &el;
public:
    GreaterWidget(qreal width, qreal height,  LDElement &el, QGraphicsItem *parent=nullptr): QGraphicsWidget(parent),width(width),height(height), el(el)
    {
...
public:
    ...
    {
        QSvgRenderer renderer(QString(":/images/more.svg"),this);
...
GreaterElement::GreaterElement(qreal cell_width, qreal cell_height):LDElement (cell_width,cell_height)
{
    item = new GreaterWidget(cell_width,cell_height,*this);
    item->setToolTip("Больше чем");
    row_span = 2;
    input_count=2;
    setType("greather than");
}

LDElement *GreaterElement::clone()
{
    auto* el = new GreaterElement(cell_width,cell_height);
