...
class EqualWidget: public QGraphicsWidget
{
    qreal width, height;
    LDElement &el;
public:
    EqualWidget(qreal width, qreal height,  LDElement &el, QGraphicsItem *parent=nullptr): QGraphicsWidget(parent),width(width),height(height), el(el)
    {
...
public:
    ...
    {
        QSvgRenderer renderer(QString(":/images/equal.svg"),this);
...
EqualElement::EqualElement(qreal cell_width, qreal cell_height):LDElement (cell_width, cell_height)
{
    item = new EqualWidget(cell_width,cell_height,*this);
    item->setToolTip("Равенство");
    row_span = 2;
    setType("equal");
    input_count=2;
}

LDElement *EqualElement::clone()
{
    auto* el = new EqualElement(cell_width,cell_height);
