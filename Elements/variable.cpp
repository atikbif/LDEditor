...
class VariableWidget: public QGraphicsWidget
{
    qreal width, height;
    LDElement &el;
public:
    VariableWidget(qreal width, qreal height,  LDElement &el, QGraphicsItem *parent=nullptr): QGraphicsWidget(parent),width(width),height(height), el(el)
    {
...
public:
    ...
    void paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget) override
    {
        ...
        QSvgRenderer renderer(QString(":/images/variable.svg"));
...
Variable::Variable(qreal cell_width, qreal cell_height): LDElement (cell_width,cell_height)
{
    item = new VariableWidget(cell_width,cell_height,*this);
    item->setToolTip("Переменная");
    setType("variable");
    needConnect = true;
    input_count=1;
}

LDElement *Variable::clone()
{
    auto* el = new Variable(cell_width,cell_height);
