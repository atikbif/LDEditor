...
class DivideWidget: public QGraphicsWidget
{
    qreal width, height;
    LDElement &el;
public:
    DivideWidget(qreal width, qreal height,  LDElement &el, QGraphicsItem *parent=nullptr): QGraphicsWidget(parent),width(width),height(height), el(el)
    {
...
public:
    ...
    void paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget) override
    {
        ...
        QSvgRenderer renderer(QString(":/images/divide.svg"),this);
...
DivideElement::DivideElement(qreal cell_width, qreal cell_height):LDElement(cell_width,cell_height)
{
    item = new DivideWidget(cell_width,cell_height,*this);
    item->setToolTip("Деление");
    row_span = 2;
    setType("divide");
    input_count=2;
}

LDElement *DivideElement::clone()
{
    auto* el = new DivideElement(cell_width,cell_height);
