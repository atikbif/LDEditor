...
class RelayWidget: public QGraphicsWidget
{
    qreal width, height;
    LDElement &el;
public:
    RelayWidget(qreal width, qreal height,  LDElement &el, QGraphicsItem *parent=nullptr): QGraphicsWidget(parent),width(width),height(height), el(el)
    {
...
public:
    ...
    void paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget) override
    {
        ...
        QSvgRenderer renderer(QString(":/images/rele.svg"));
...
Relay::Relay(qreal cell_width, qreal cell_height):LDElement (cell_width,cell_height)
{
    item = new RelayWidget(cell_width,cell_height,*this);
    item->setToolTip("Реле");
    row_span = 2;
    setType("relay");
    needConnect = true;
    onlyWrite = true;
...
LDElement *Relay::clone()
{
    auto* el = new Relay(cell_width,cell_height);
