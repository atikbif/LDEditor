...
class LessWidget: public QGraphicsWidget
{
    qreal width, height;
    LDElement &el;
public:
    LessWidget(qreal width, qreal height,  LDElement &el, QGraphicsItem *parent=nullptr): QGraphicsWidget(parent),width(width),height(height), el(el)
    {
...
public:
    ...
    {
        QSvgRenderer renderer(QString(":/images/less.svg"),this);
...
LessElement::LessElement(qreal cell_width, qreal cell_height):LDElement(cell_width,cell_height)
{
    item = new LessWidget(cell_width,cell_height,*this);
    item->setToolTip("Меньше чем");
    row_span = 2;
    input_count=2;
    setType("less than");
}

LDElement *LessElement::clone()
{
    auto* el = new LessElement(cell_width,cell_height);
