...
ModbusRequestList::ModbusRequestList()
{
...
{
    QStringList res;
    res.append("uint16_t mmb[64]={0};\n");
    res.append("uint8_t err_mod[256]={0};\n");
    return res;
}

QStringList ModbusRequestList::getResult(int canNum)
{
    QStringList res;
    QStringList reqBody = getReqText(canNum);
    res += "const uint16_t canal" + QString::number(canNum) + "_req_count=" + QString::number(reqs.count())+";";
    res += "const uint16_t canal" + QString::number(canNum) + "_modbus_delay = " + QString::number(ModbusVarsStorage::getInstance().getPeriod(canNum))+";";
    res += "";
    res += reqBody;
    return res;
}

void ModbusRequestList::createRequests(int canNum)
{
    reqs.clear();
    ...
    sortVarsByMemAddress(vars);
    for(int i=0;i<vars.count();++i) {
        VarReq vr;
        QSharedPointer<ModbusVar> v = vars.at(i);
        Request req;
        req.netAddress = v->getNetAddress();
        ...
        req.cnt = 1;
        vr.byteOffset = 0;
        vr.varName = "MODB"+QString::number(v->getNumber());
        req.reqVars.append(vr);
        addRequestToList(req,canNum);
    }
}
...
void ModbusRequestList::sortVarsByMemAddress(QVector<QSharedPointer<ModbusVar>> &vars)
{
...
void ModbusRequestList::addRequestToList(const ModbusRequestList::Request &r,int canNum)
{
    ...
    if(reqs.count()) {
        for(int i=reqs.count()-1;i>=0;--i) {
            ...
            if((req.netAddress==r.netAddress)&&(req.cmd==r.cmd)&&(req.memAddress<r.memAddress)&&r.reqVars.count()) {
                ...
                if((requiredCount <= ModbusVarsStorage::getInstance().getMaxLength(canNum)) && (spaceLength <= ModbusVarsStorage::getInstance().getMaxUnusedSpace(canNum))) {
...
{
    QStringList res;
    res += "#include \"modbus_master.h\"";
    return res;
}

QStringList ModbusRequestList::getReqText(int canNum)
{
    QStringList res;
    createRequests(canNum);
    for(int i=0;i<reqs.count();++i) {
        res += "const mvar canal" + QString::number(canNum) + "_req" + QString::number(i+1) + "_vars[] = {";
    ...
    res+="";
    res += "const mvar_reqs canal" + QString::number(canNum) + "_mvar_reqs[] = {";
    for(int i=0;i<reqs.count();++i) {
        Request r = reqs.at(i);
        QString reqStr = "{\"";
        ...
        reqStr += QString(", ") + "canal" + QString::number(canNum) + "_req" + QString::number(i+1) + "_vars, ";
